...
func TestNewServer_HTTPInvalidPath(t *testing.T) {
	server, err := NewServer(
		config.MCPServer{
			Name:    "Test",
			Version: "1.0.0",
	...
	require.Error(t, err)
	assert.Nil(t, server)
	assert.Contains(t, err.Error(), "http path must start with /")
}

func TestNewServer_HTTPNegativeTimeout(t *testing.T) {
	server, err := NewServer(
		config.MCPServer{
			Name:    "Test",
			Version: "1.0.0",
	...
	require.Error(t, err)
	assert.Nil(t, server)
	assert.Contains(t, err.Error(), "http read timeout must not be negative")
}
